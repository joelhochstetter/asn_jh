function [gamma, gamma_vals, RMS_errors, new_t, size_t_ave, size_t_var] = scaleCollapse(lifeAv, lifeFreq, time_t, size_t, Tmax, minTime, minFreq)
%{
    Input: 
...
%}
    ...
    selected = (lifeAv >= minTime) & (lifeFreq >= minFreq) & (lifeAv <= Tmax);
    lifeAv     = lifeAv(selected);
    ...
    for g = 1:numel(gamma_vals)
        for t = 1:numel(new_t)
            ...
            for d = 1:numel(lifeAv)
                m_sz       = m_sz +size_t_int{d}(t)/lifeAv(d)^(gamma_vals(g));
                m_sq_sz = m_sq_sz + (size_t_int{d}(t)/lifeAv(d)^(gamma_vals(g)))^2;
            end
            m_sz         = m_sz/numel(lifeAv);
            m_sq_sz   = m_sq_sz/numel(lifeAv);
            size_t_var(g, t) = m_sq_sz - m_sz^2;
            size_t_ave(g,t) = m_sz;
            RMS_errors(g) = RMS_errors(g) + size_t_var(g, t);
        end
        RMS_errors(g) = RMS_errors(g)*lifeAv(d)^(2*gamma_vals(g));
    end
    ...
    for i = 1:numel(lifeAv)
        hold on;        
        plot(re_tm{i}, size_t{i}/lifeAv(i)^gamma)
    end
