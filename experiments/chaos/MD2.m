...
SimulationOptions.ContactNodes = [73, 30]; % only really required for preSet, other modes will overwrite this
...
Components.ComponentType       = 'tunnelSwitch2'; % 'atomicSwitch' \ 'memristor' \ 'resistor', \'tunnelSwitch' ,  \'quantCSwitch'
​
Components = initializeComponents(Connectivity.NumberOfEdges,Components);
​
​
%% Initialize stimulus:
Stimulus.BiasType              = 'ACsaw';           % 'DC' \ 'AC' \ 'DCandWait' \ 'Ramp' \ 'ACsaw'
switch Stimulus.BiasType
    case 'DC'
        Stimulus.Amplitude = 1.5;%0.0448973; %0.045;  % (Volt)
    case {'AC', 'ACsaw'}
        Stimulus.Frequency       = 2; % (Hz)
        Stimulus.Amplitude       = 3; % (Volt)
        Stimulus.Phase           = 0;   % (Rad)
    case 'DCandWait'
        Stimulus.OffTime      = 0.95*SimulationOptions.T; %SimulationOptions.T/2; % (sec)
        Stimulus.AmplitudeOn  = 1.5;                   % (Volt)
        Stimulus.AmplitudeOff = 0.08;                 % (Volt)
    case 'Ramp'
...
%% Output Conductance Time Series (Yes, Output.networkResistance is actually the conductance.)
Conductance = Output.networkResistance;
%% Graphics:
if SimulationOptions.takingSnapshots
    % What to plot:
    whatToPlot = struct(...
