function DC_sweep_V_InitState(idx, saveFolder, numInitStates, Gmin, Gmax) 
    
    Gidx = mod((idx-1), numInitStates) + 1; 
    Vidx    = floor((idx-1)/numInitStates) + 1;
     initCon = 10.^linspace(log10(Gmin), log10(Gmax), numInitStates);
    
    DC_Vsweep_for_cluster(Vidx, saveF1, 0.5*0.01*9, 3.0*0.01*9, 0.025*0.01*9, nets(netIdx).name, 0 , '.', -1, 600, true)
