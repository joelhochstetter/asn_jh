...
for j = 1:Nbs
    ...
    for i = 1:numel(Nvals)
        critResults{i,j} = load(strcat2({baseFolder, 'NumWires', num2str(Nvals(i), '%04.f'), '/bs', bs, '/critResults.mat'}));
    ...
    %% IEI
    figure('visible', 'off');
    errorbar(Nvals, IEItau(:,j), IEIdta(:,j), '--o');
    xlabel('N_w')
    ylabel('\alpha')
    yyaxis right;
    semilogy(Nvals, meanIEI(:,j), 'o-');
    ylabel('<IEI>')
    ...
    hold on;
    plot(Nvals, Suct(:,j), 'k--');
    plot(Nvals, kingAv(:,j), 'r^', 'MarkerSize', 10)
    ylabel('cut-off')
    ...
    %% Gamma
    figure('visible', 'off');
    errorbar(Nvals,x1(:,j), dx1(:,j));
    hold on;
    errorbar(Nvals,x2(:,j), dx2(:,j));
    errorbar(Nvals,x3(:,j), dx3(:,j));
    xlabel('N_w')
    ylabel('1/\sigma\tau\nu')
...
for j = 1:N
    saveFolder = strcat(baseFolder, '/AvCompare/Nw', num2str(Nvals(j), '%04.f'),'/');
    mkdir(saveFolder);
