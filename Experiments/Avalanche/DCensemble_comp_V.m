...
%         critResults{i,j} = load(strcat2({baseFolder, 'V', Vvals(i), '/bs', bs, '/critResults.mat'}));
    ...
    %% IEI
    figure('visible', 'off');
    errorbar(Vstar, IEItau(:,j), IEIdta(:,j), '--o');
    xlabel('V^*')
    ylabel('\alpha')
    yyaxis right;
    semilogy(Vstar, meanIEI(:,j), 'o-');
    ylabel('<IEI>')
    ...
    %% Gamma
    figure('visible', 'off');
    errorbar(Vstar,x1(:,j), dx1(:,j));
    hold on;
    errorbar(Vstar,x2(:,j), dx2(:,j));
    errorbar(Vstar,x3(:,j), dx3(:,j));
    xlabel('V^*')
    ylabel('1/\sigma\tau\nu')
...
for j = 1:N
    saveFolder = strcat(baseFolder, '/AvCompare/Vstar', num2str(Vstar(j)),'/');
    mkdir(saveFolder);
