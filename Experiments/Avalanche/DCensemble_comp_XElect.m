...
for j = 1:Nbs
    ...
    for i = 1:numel(Evals)
        critResults{i,j} = load(strcat2({baseFolder, 'XESize', num2str(Evals(i)), '/bs', bs, '/critResults.mat'}));
    ...
    %% dG
    figure('visible', 'off');
    errorbar(Evals, dGalpha(:,j), dGdalph(:,j), '--o');
    xlabel('E_X frac')
    ylabel('\alpha')
    yyaxis right;
    semilogy(Evals, meanG(:,j), '-o');
    ylabel('<G>')
    vline(1, 'k')
    ...
    %% PSD
    figure('visible', 'off');
    errorbar(Evals, PSDbeta(:,j), PSDdbet(:,j), '--o');
    xlabel('E_X frac')
    ylabel('\beta')
    title('PSD exponent')
    ...
    %% IEI
    figure('visible', 'off');
    errorbar(Evals, IEItau(:,j), IEIdta(:,j), '--o');
    xlabel('E_X frac')
    ylabel('\alpha')
    yyaxis right;
    semilogy(Evals, meanIEI(:,j), 'o-');
    ylabel('<IEI>')
    ...
    hold on;
    plot(Evals, Suct(:,j), 'k--');
    plot(Evals, kingAv(:,j), 'r^', 'MarkerSize', 10)
    ylabel('cut-off')
    ...
    %% Gamma
    figure('visible', 'off');
    errorbar(Evals,x1(:,j), dx1(:,j));
    hold on;
    errorbar(Evals,x2(:,j), dx2(:,j));
    errorbar(Evals,x3(:,j), dx3(:,j));
    xlabel('E_X frac')
    ylabel('1/\sigma\tau\nu')
...
for j = 1:N
    saveFolder = strcat(baseFolder, '/AvCompare/Ex', num2str(Evals(j)),'/');
    mkdir(saveFolder);
