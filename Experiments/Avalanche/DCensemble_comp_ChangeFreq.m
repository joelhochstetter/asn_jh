...
for j = 1:Nbs
    ...
    %% dG
    figure('visible', 'off');
    errorbar(fvals, dGalpha(:,j), dGdalph(:,j), '--o');
    xlabel('f (Hz)')
    ylabel('\alpha')
    yyaxis right;
    semilogy(fvals, meanG(:,j), '-o');
    ylabel('<G>')
    ...
    %% PSD
    figure('visible', 'off');
    errorbar(fvals, PSDbeta(:,j), PSDdbet(:,j), '--o');
    xlabel('f (Hz)')
    ylabel('\beta')
    title('PSD exponent')
    ...
    %% IEI
    figure('visible', 'off');
    errorbar(fvals, IEItau(:,j), IEIdta(:,j), '--o');
    xlabel('f (Hz)')
    ylabel('\alpha')
    yyaxis right;
    semilogy(fvals, meanIEI(:,j), 'o-');
    ylabel('<IEI>')
    ...
    hold on;
    plot(fvals, Suct(:,j), 'k--');
    plot(fvals, kingAv(:,j), 'r^', 'MarkerSize', 10)
    ylabel('cut-off')
    ...
    %% Gamma
    figure('visible', 'off');
    errorbar(fvals,x1(:,j), dx1(:,j));
    hold on;
    errorbar(fvals,x2(:,j), dx2(:,j));
    errorbar(fvals,x3(:,j), dx3(:,j));
    xlabel('f (Hz)')
    ylabel('1/\sigma\tau\nu')
    ...
    ylabel('P(T)')
    title('IEI')
    leg = legend(num2str(fvals), 'location', 'best');
    title(leg,'f (Hz)')
...
for j = 1:N
    saveFolder = strcat(baseFolder, '/AvCompare/freq', num2str(fvals(j)),'/');
    mkdir(saveFolder);
